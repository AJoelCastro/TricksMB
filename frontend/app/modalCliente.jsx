...
const Modal = () => {
  ...
  const handleCrearCliente = async () => {
    try {
      ...
      if (
        !Object.values(datosCliente).every(
  ...
  return (
    <KeyboardAvoidingView behavior='padding' className='p-2 bg-white h-full'>
      ...
      <View className='flex-row justify-center gap-4 mt-4'>
        ...
        >
          <Icon name='person' size={20} color='#634AFF' />
          <Text className='text-[#634AFF]'>Cliente Natural</Text>
        </Pressable>
        <Pressable
          onPress={() => setTipoCliente('juridico')}
          className='bg-gray-100 px-4 py-2 rounded-md w-[45%] gap-2'
        >
          <Icon name='business' size={20} color='#634AFF' />
          <Text className='text-[#634AFF]'>Cliente Juridico</Text>
        </Pressable>
      </View>
      {tipoCliente === 'natural' && (
