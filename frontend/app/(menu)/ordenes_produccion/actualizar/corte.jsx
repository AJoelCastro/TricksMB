    ...
    const cargarDetallePedido = async () => {
      try {
        ...
        const dataCaracteristicas =
          await CaracteristicasService.getAllCaracteristicasById(
            idDetallePedido
          );
        console.log('dataCaracteristicas', dataCaracteristicas);
        const dataNueva = dataCaracteristicas.caracteristicas.filter(
          item => item.Estado === 1
        );
        console.log('dataNueva', dataNueva);
        const filasTransformadas = transformarDatos(
  ...
  const updatePedido = async () => {
    try {
      for (const fila of dataDetalleAreaTrabajo) {
        ...
        const data =
          await CaracteristicasService.getCaracteristicaByIdCaracteristicas(
            idCaracteristicas
          );
        if (
        ...
        const editarDAT = await DetalleAreaTrabajoService.updatePedido(
          idCaracteristicas,
          datos
        );
    ...
    const obtenerDetalleAreaTrab = async () => {
      try {
        ...
        const filasTransformadas =
          transformarDatosDetalleAreaTrabajo(dataExtra);
        setDataDetalleAreaTrabajo(filasTransformadas);
      } catch (error) {
        console.error(
          'Error al obtener los detalles del area de trabajo:',
          error
        );
      }
    };
  ...
  const obtenerEstadosDetalleAreaTrabajo = async () => {
    try {
      ...
      if (actualizar === true) {
        ...
        const updateAreaTrabajo =
          await DetalleAreaTrabajoService.createDetalleAreaTrabajo(
            nomArea,
            codigoPedido
          );
                  ...
                  <View className='w-[25%]'>
                    <TextInput
                      ...
                      onChangeText={text => {
                        ...
                        if (
                          Number(text) + Number(nuevasFilas[index].terminado) >
                          filas[index].pares
                        ) {
                          alert(
                            'El  total no puede ser mayor a la cantidad de pares'
                          );
