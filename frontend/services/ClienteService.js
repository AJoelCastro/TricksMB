...
const ClienteService = {
  crearCliente: async (datosCliente, tipo) => {
    try {
      ...
      const endpoint =
        tipo === 'natural' ? 'cliente/natural' : 'cliente/juridico';
  ...
  obtenerClientes: async () => {
    try {
      ...
      if (error.response) {
        ...
        throw new Error(
          error.response.data.error ||
            `Error al obtener todos los clientes: ${error.response.status}`
        );
      } else if (error.request) {
        console.log('error.request', error.request);
        // No hubo respuesta del servidor
        ...
        // Otro tipo de error
        throw new Error(
          'Ocurrió un error inesperado al obtener todos los clientes.'
        );
      }
    }
  },
  buscarCliente: async (tipoCliente, identificador) => {
    try {
      ...
      const endpoint =
        tipoCliente === 'natural' ? `buscar/natural` : `buscar/juridico`;
  ...
  getClienteByCodigoPedido: async codigoPedido => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/cliente/cliente/${codigoPedido}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
